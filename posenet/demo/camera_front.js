...
function detectPoseInRealTime(video, net) {
	...
	async function poseDetectionFrame() {
		...
		document.getElementById("arm-ng-sound-file").volume = elmVolume;
		document.getElementById("arm-ok-sound-file").volume = elmVolume;
		...
		poses.forEach(({ score, keypoints }) => {
			if (score >= minPoseConfidence) {
				if (guiState.output.showPoints) {
					...
					// 腕が床と垂直になっているか判定する．
					// 肘と手首のx座標の差
					let leftarmPosition = Math.abs(keypoints[leftWrist].position.x - keypoints[leftElbow].position.x);
					// 左腕の判定
					if (leftarmPosition < armlow) {
						leftarm = "OK";
					}
					else if (leftarmPosition > armhigh) {
						leftarm = "NG";
					}
					else {
					...
					// 肘と手首のx座標の差
					let rightarmPosition = Math.abs(keypoints[rightWrist].position.x - keypoints[rightElbow].position.x);
					// 右腕の判定
					if (rightarmPosition < armlow) {
						rightarm = "OK";
					}
					else if (rightarmPosition > armhigh) {
						rightarm = "NG";
					}
					else {
					...
					// 腕の判定
					if ((leftarmArr[0] == "OK") && (rightarmArr[0] == "OK")){
						arm = "OK";
					} 
					else if ((leftarmArr[0] == "NG") && (rightarmArr[0] == "NG")) {
						arm = "NG"
					}
					else {
						arm = armArr[0];
					...
					armArr.pop();
					armArr.unshift(arm);
					document.getElementById("JudgementArm").innerHTML = "腕: " + arm;
					...
					document.getElementById("JudgementChestCount").innerHTML = "カウントの回数: " + chestCount;
				}
			}
		});
	...
	function armNgSound() {
		// 初回以外は音声ファイルを巻き戻す(再生位置[秒]を0に設定する)
		if (typeof (document.getElementById('arm-ng-sound-file').currentTime) != 'undefined') {
			document.getElementById('arm-ng-sound-file').currentTime = 0;
		}
		//音声ファイルを再生する
		document.getElementById('arm-ng-sound-file').play();
	}

	function armOkSound() {
		// 初回以外は音声ファイルを巻き戻す(再生位置[秒]を0に設定する)
		if (typeof (document.getElementById('arm-ok-sound-file').currentTime) != 'undefined') {
			document.getElementById('arm-ok-sound-file').currentTime = 0;
		}
		//音声ファイルを再生する
		document.getElementById('arm-ok-sound-file').play();
	}
